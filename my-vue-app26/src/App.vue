...
const saarcCountries = reactive([
  ...
  { name: "Pakistan", capital: "Islambad" },
  { name: "Maldives", capital: "Male" },
]);

const capitals = ['Dhaka', 'Kathmandu', 'Thimphu', 'Colombo', 'New Delhi', 'Islambad', 'Male'];
  ...
  <section class="mx-auto container text-left">
    ...
    <div class="my-5 border border-gray-400 p-5" v-for="country, index in saarcCountries" :key="country.name">
      ...
      <template v-for="capital, index in capitals">
        <input :name="country.name" type="radio" :value="capital" v-model="country.answer" id="">
